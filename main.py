...
def summarize_quirky(text):
    ...
    prompt = (
        "You're a quirky, upbeat radio weather announcer. "
        "Summarize up to 3 major weather alerts in a humorous, informal, and vivid way. "
        "Your summary will be read out loud, so only use plain punctuation like periods, commas, exclamation marks, and question marks. "
        "Special symbols (such as *, @, #, or emojis), are not allowed under any circumstances, so avoid them at all cost. Write all numbers out as words and avoid abbreviations. "
        f"Here are the alerts:\n{text}"
    )
